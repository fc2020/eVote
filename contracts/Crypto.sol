...
contract Crypto {
    ...
    function verifyDL(uint[2] memory xG, uint[3] memory proof) public view returns (bool) {
        ...
        return (temp[0]==zG[0] && temp[1] == zG[1]);
    }
    ...
    function verifyZeroOrOne(uint[2] memory c, uint[2] memory H, uint[7] memory proof) public view returns (bool) {
        uint x = uint(keccak256(abi.encodePacked(c,proof[0],proof[1],proof[2],proof[3])));
        uint[2] memory fza = commit(proof[4],proof[5],H);
        uint[2] memory zb = commit(0, proof[6],H);
